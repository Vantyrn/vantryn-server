...
function checkEnv() {
  ...
  if (process.env.DEV_BYPASS === 'on') {
    ...
    if (isProd) {
      logger.error('env.dev_bypass_on_in_production');
      console.error(msg);
    } else {
  ...
  if (missingCritical.length) {
    ...
    logger.error('env.critical_missing', { missing: missingCritical, willExit: isProd });
    console.error('\n' + '='.repeat(72) + '\n' + banner);
    if (isProd) {
      console.error('Refusing to boot in production with missing secrets (would run insecurely).\n' + '='.repeat(72) + '\n');
      process.exit(1);
    } else {
