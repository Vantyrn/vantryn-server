      ...
      if (!global.__workersInitialized) {
        ...
        const orderSlaWorker = new Worker('orderSla', async (job) => {
          ...
          if (type === 'vendor_accept' && (order.status === 'pending_vendor' || order.status === 'Awaiting Vendor Acceptance')) {
            ...
            // orderId, not id — `id` is undeclared here, so this line threw a ReferenceError
            // that killed the worker before the SLA metric and breach row below were ever
            // written (vendor_breaches has 0 rows).
            if (io) io.of('/admin').to('admin_global').emit('vendor_sla_breach', { orderId, type: 'NO_ACCEPTANCE' });
