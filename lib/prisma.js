...
// '../../.env' resolved to d:/Vantryn/.env — one level ABOVE the server, where no
// .env has ever existed — so this fallback silently did nothing and every locally
// run script died with "DATABASE_URL is not defined". The file is vantryn-server/.env.
// Hosted envs (Railway) set DATABASE_URL directly and never reach this branch.
  ...
  let profile = await withRetry(async () => {
    ...
    if (phoneNumber) {
      ...
      if (p) {
        ...
        // Only CLAIM an unset role — never overwrite an existing one. This used to
        // hard-set 'CUSTOMER', so a vendor who opened the customer app with the same
        // phone number had their profile silently demoted; every role === 'VENDOR'
        // check server-side then failed for them (KYC gate, phone verification,
        // profile status self-heal) while their vendor record still said ACTIVE.
        // A profile can legitimately own both records, so role is only a label for
        // which app created it — capability checks must key off the relation.
        return await prisma.profile.update({
          where: { id: p.id },
          data: { firebaseUid: uid, ...(p.role ? {} : { role: 'CUSTOMER' }) },
          include: { customer: { include: { ageVerification: true } } }
        });
      }
    }
  ...
  if (!profile.customer) {
    ...
    if (!customer) {
        try {
            ...
            // Race-condition safe: on first login the app fires several authed
            // requests at once (cart/address/profile/fcm). They all see no
            // customer and try to create one → only the first wins; the rest hit
            // P2002 (unique profile_id or email). Re-fetch the row that now exists.
            if (createError.code === 'P2002') {
                console.log('[PRISMA] Customer creation race-condition, re-fetching...');
                // profileId is unique (one customer per profile), so this returns
                // THIS profile's customer that the winning concurrent request just
                // created. If it's null, the conflict was a genuine email clash
                // with another account — let that surface rather than adopting a
                // customer that belongs to a different profile.
